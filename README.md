<div id="header" align="center">
  <img src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExc3A2anlneTA3ajAxZXYxOGx4Y2ZqbWtvYjhwdGRhbmlqY2JxcmlzYSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/0lfqHNZwWM1hOvJ9CX/giphy.gif" width="100"/>
</div>
<div id="views" align="center">
  <img src="https://komarev.com/ghpvc/?username=Joker-7-7&style=flat-square&color=blue"/>
  <h1>
    hey there
    <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30px"/>
  </h1>
</div>
<div align="center">
  <img src="https://media.giphy.com/media/dWesBcTLavkZuG35MI/giphy.gif" width="600" height="300"/>
</div>

---
### 👨‍💻 About Me

C++ developer specializing in medical image processing and visualization.
I work with ITK, VTK, and Qt/QML, developing tools for 3D segmentation, volume rendering, and DICOM data analysis.

🔬 Key areas:

- 3D segmentation (Level Set, Fast Marching, Sparse Field)

- Compute optimization and memory consumption reduction

- Working with DICOM, MHA, and medical tomography data

- Integration of ITK algorithms into VTK/Qt applications

- Curved MPR, panoramic reconstructions, volume cropping

- Marching Cubes and 3D visualization


Focus on the performance, architecture of medical imaging systems, and deep integration of image processing algorithms into applications.

---
### :hammer_and_wrench: Languages and Tools :

<div>
  <img src="https://github.com/devicons/devicon/blob/master/icons/cplusplus/cplusplus-original.svg" title="Cplusplus" alt="Cplusplus" width="40" height="40"/>&nbsp;
 <img src="https://github.com/devicons/devicon/blob/master/icons/qt/qt-original.svg" title="qt" alt="qt" width="40" height="40"/>&nbsp;
  <img src="https://github.com/devicons/devicon/blob/master/icons/opengl/opengl-plain.svg" title="opengl" alt="opengl" width="40" height="50"/>&nbsp;
  <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/76/Visualization_Toolkit_logo.svg/3840px-Visualization_Toolkit_logo.svg.png" title="vtk" alt="vtk" width="50" height="40"/>&nbsp;
   <img src="https://discourse.itk.org/uploads/default/original/1X/83aa25abdcbd323ce491ff99db5dfe2e59d7fd4d.png" title="itk" alt="itk" width="60" height="60"/>&nbsp;
  <img src="https://www.vikingsoftware.com/wp-content/uploads/2024/02/QML.png" title="qml" alt="qml" width="40" height="40"/>&nbsp;
</div>
---

### :fire: My Stats :
[![GitHub Streak](http://github-readme-streak-stats.herokuapp.com?user=Joker-7-7&theme=dark&background=000000)](https://git.io/streak-stats)

